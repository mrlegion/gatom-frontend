...
import { OrganizationUpdateSchema, TOrganizationUpdateFormValue } from '@/schemas/organizations'
import { TUsage } from '@/types/common'
import type { IOrganization } from '@/types/organizations'

interface IOrganizationUpdateDialogProps {
	item: IOrganization
	usage: TUsage
	onlyIcon?: boolean
}

export function OrganizationUpdateDialog({ item, usage, onlyIcon = false }: IOrganizationUpdateDialogProps) {
	const [isOpen, setIsOpen] = useState<boolean>(false)
	const { updateOrganization, isUpdateOrganizationPending } = useUpdateOrganization(item.id, usage)

	const form = useForm<TOrganizationUpdateFormValue>({
	...
	const onSubmit = async (values: TOrganizationUpdateFormValue) => {
	...
	const handleOpenChange = (nextOpen: boolean) => {
		if (!nextOpen && isUpdateOrganizationPending) return
		setIsOpen(nextOpen)
	}
	...
	return (
		<Dialog open={isOpen} onOpenChange={handleOpenChange}>
			<div className='flex'>
				<DialogTrigger asChild>
					<Button variant='outline' className='pr-5 pl-5'>
						<Edit className={onlyIcon ? 'h-4 w-4' : ''} /> {onlyIcon ? '' : 'Редактировать'}
					</Button>
				</DialogTrigger>
			</div>
			<DialogContent className='sm:max-w-sm'>
				...
				<DialogFooter>
					<DialogClose asChild>
						<Button variant='outline' className='pr-5 pl-5' disabled={isUpdateOrganizationPending}>
							<XIcon /> Отмена
						</Button>
					</DialogClose>
					<Button
						...
						className='pr-5 pl-5'
					>
						<Save /> {isUpdateOrganizationPending ? 'Обновление...' : 'Обновить'}
					</Button>
				</DialogFooter>
			</DialogContent>
		</Dialog>
